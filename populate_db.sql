...
VALUES(
  ...
  2023,
  (SELECT idGenre FROM Genres WHERE category = "Sci-Fi"),
...
(
  "Avengers: Endgame",
  2019,
  (SELECT idGenre FROM Genres WHERE category = "Action"),
...
(
  "Home Alone",
  1990,
  (SELECT idGenre FROM Genres WHERE category = "Comedy"),
  "A movie about a boy left at home alone",
  6.8
);
...
CREATE TABLE AudienceReviews(
  ...
  FOREIGN KEY (idMovie) REFERENCES Movies(idMovie)
    ON DELETE CASCADE,
  FOREIGN KEY (idAudience) REFERENCES Audiences(idAudience)
    ON DELETE CASCADE,
  CONSTRAINT movie_audience UNIQUE(idMovie, idAudience)
);

INSERT INTO AudienceReviews(
  review,
  idMovie,
  idAudience,
  stars
...
VALUES(
  ...
  (SELECT idMovie FROM Movies WHERE title = "Spider-Man: Across the Spider Verse"),
  (SELECT idAudience FROM Audiences WHERE firstName = "John" AND lastName = "Adams"),
  5
),
(
  "Mid",
  (SELECT idMovie FROM Movies WHERE title = "Spider-Man: Across the Spider Verse"),
  (SELECT idAudience FROM Audiences WHERE firstName = "Adam" AND lastName = "James"),
  3
),
(
  "Hate the cliff-hanger, for that 1 star.",
  (SELECT idMovie FROM Movies WHERE title = "Spider-Man: Across the Spider Verse"),
  (SELECT idAudience FROM Audiences WHERE firstName = "Jawan" AND lastName = "Curry"),
  1
);
...
(
   (SELECT idMovie FROM Movies WHERE title = "Spider-Man: Across the Spider Verse"),
   (SELECT idActor FROM Actors WHERE firstName = "Shameik" AND lastName = "Moore")
),
(
   (SELECT idMovie FROM Movies WHERE title = "Spider-Man: Across the Spider Verse"),
   (SELECT idActor FROM Actors WHERE firstName = "Hailee" AND lastName = "Steinfeld")
),
(
   (SELECT idMovie FROM Movies WHERE title = "Spider-Man: Across the Spider Verse"),
   (SELECT idActor FROM Actors WHERE firstName = "Brian" AND lastName = "Henry" AND middleName = "Tyree")
);
...
INSERT INTO Movies_has_Directors(
  idMovie,
  idDirector
)
VALUES(
  (SELECT idMovie FROM Movies WHERE title = "Spider-Man: Across the Spider Verse"),
  (SELECT idDirector FROM Directors WHERE firstName = "Joaquim" AND lastName = "Dos Santos")
),
(
  (SELECT idMovie FROM Movies WHERE title = "Spider-Man: Across the Spider Verse"),
  (SELECT idDirector FROM Directors WHERE firstName = "Kemp" AND lastName = "Powers")
),
(
  (SELECT idMovie FROM Movies WHERE title = "Spider-Man: Across the Spider Verse"),
  (SELECT idDirector FROM Directors WHERE firstName = "Justin" AND lastName = "Thompson")
);
